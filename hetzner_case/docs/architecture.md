...
node.hetzner_demo.io/pool: control-plane

# General workload nodes (CX41 cloud VMs)
node.hetzner_demo.io/pool: general
node.hetzner_demo.io/type: cloud

# Storage-pool nodes (CX51 cloud VMs — Elasticsearch + media-download)
node.hetzner_demo.io/pool: storage
node.hetzner_demo.io/type: cloud
```
...
├── hetzner_demo-web             ← Web App
├── hetzner_demo-search          ← Media Search Service + ElasticSearch
└── hetzner_demo-download        ← Media Download Service
```
...
### IMAGO Web App (hetzner_demo-web)
...
- **Node placement:** General cloud nodes (`node.hetzner_demo.io/pool: general`) — bare metal is not required; `hcloud-volumes` cannot attach to bare metal
...
- **Credentials:** ECK auto-creates `hetzner_demo-media-search-es-elastic-user` secret; no manual password management
- **Ingress:** Not externally exposed — only reachable from `hetzner_demo-web` namespace via NetworkPolicy
- **Backup:** Elasticsearch snapshots to Hetzner Object Storage daily

### Media Download Service (hetzner_demo-download)

- **Deployment:** 2–4 replicas
- **Ingress:** `dl.hetzner_demo.de` → TLS
...
- Wildcard certificate for `*.hetzner_demo.de` for flexibility
