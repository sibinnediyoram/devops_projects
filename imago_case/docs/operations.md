...
1. Check cluster health:
   kubectl exec -n imago-search elasticsearch-0 -- \
...
3. If node is down — check pod status:
   kubectl get pods -n imago-search -l app=elasticsearch

4. If pod is in CrashLoopBackOff — check logs:
   kubectl logs -n imago-search elasticsearch-0 --previous
...
1. Check pod status:
   kubectl get pods -n imago-web

2. Check recent logs:
   kubectl logs -n imago-web -l app=web-app --tail=100

3. Check if Search Service is healthy:
   kubectl get pods -n imago-search

4. Check if it's a deployment issue (recent rollout):
   kubectl rollout history deployment/web-app -n imago-web
   # If yes, rollback:
   kubectl rollout undo deployment/web-app -n imago-web
...
2. SSH to node (via bastion):
   ssh -J bastion.imago.de ubuntu@<node-ip>
...
kubectl set image deployment/web-app web-app=registry.imago.de/web-app:<hotfix-tag> -n imago-web
# 3. Verify:
kubectl rollout status deployment/web-app -n imago-web
...
kubectl rollout restart statefulset/elasticsearch -n imago-search
